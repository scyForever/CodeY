...
IGNORED_PATH_NAMES = {".git", ".codey", "__pycache__", ".pytest_cache", ".ruff_cache", ".venv", "venv"}
...
class WorkspaceContext:
    ...
    def build(cls, cwd, repo_root_override=None):
        ...
        try:
            if cwd.exists() and repo_root.exists() and cwd.samefile(repo_root):
                cwd = repo_root
        except OSError:
            pass
        docs = {}
        ...
        for base in (repo_root, cwd):
            for name in DOC_NAMES:
                ...
                try:
                    ...
                    key = name if base == cwd else str(path)
                if key in docs:
                    continue
