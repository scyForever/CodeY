...
def test_same_session_reroutes_each_new_request(tmp_path):
    ...
    assert agent.ask("CodeY agent runtime engineering: fix prompt context") == "first"
    first_run = agent.current_task_state.run_id
    assert agent.current_task_state.route_id == "prompt-context"
    assert agent.ask("local coding assistant harness: 加强工具安全") == "second"
...
def test_prompt_xml_and_route_section_order(tmp_path):
    ...
    request = "CodeY agent runtime engineering: fix prompt context"
    agent.route_task(request, run_id="run-test", task_id="task-test")
    ...
    assert prompt.index("<selected-skill") < prompt.index("Memory:")
...
def test_budget_reduction_preserves_current_request_and_route_floor(tmp_path):
    agent = build_agent(tmp_path, ["<final>done</final>"])
    agent.route_task(
        "CodeY agent runtime engineering: fix prompt context",
        run_id="run-test",
        task_id="task-test",
    )
    ...
    for index in range(15):
        agent.record_transcript(
            {"role": "assistant", "content": f"history-{index}-" + "x" * 500, "created_at": "now"},
            turn_id=f"history-turn-{index:04d}",
        )
